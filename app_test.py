...
class ApiEndpoints(SiteTest):
    ...
    def test_api_docs(self):
        ...
        # POST - malformed data
        response = self.client.post('/docs', app.config['API_SUBDOMAIN'], headers=headers, data=bad_data)
        self.assertEqual(response.status_code, 400)

        # POST - valid data
        response = self.client.post('/docs', app.config['API_SUBDOMAIN'], headers=headers, data=valid_data_json)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json, {"success": True})
